...
The replay page accepts trainer JSON and native `.ttr` / `.ttrm`, via file picker or drag and drop. It supports player / round selection, play / pause, 0.25x to 4x speed, seeking, frame stepping, and Hold / Next display. It reconstructs the effective recording with retried and undone branches removed. All original finesse attempts remain available to Statistics and fault practice. Thinking time and manual pauses are omitted from the playback timeline. Playback uses the recorded mode layout, Hold/Next, ghost settings, placement effects and sound effects. Seeking and frame stepping are silent. Native playback shares the validated importer and its mode compatibility limits.
...
AI helper candidates and implementation order are documented in [AI-TRAINING-RESEARCH.md](TEMP/AI-TRAINING-RESEARCH.md). Stricter finesse metrics are no longer planned. Openers now has bounded local PC and T-spin continuation search; a general versus bot is not activated. Research and temporary documents belong in `TEMP/`.


## Openers, shortlist and continuation hints

Openers contains 478 public catalog entries, including 442 validated first constructions, source links, mirrored previews and local Fumen import. A star shortlists an opener and pins it above other entries. Random opening search checks shortlisted entries first and shows compatible candidates first. Shortlists and opening preferences persist in this browser.

Random opening training places the board on the left and candidate diagrams on the right. Build freely or click a diagram for assisted targets. R and Restart game deal a new seed. Single-opener practice also finds a new compatible seed. Equivalent structural variants can change piece allocations while preserving the occupied shape; the diagrams show the actual selected allocation. Keep board after construction defaults to on. Turning it off immediately repeats the completed opener with a new seed. With it on, the existing field and queue remain available for continued play.

Continuation placement hints defaults to off and can be enabled on Openers or beside the play board. After the opening, choose Perfect clear, one/two T-spin clears, or one/two T-spin doubles. The opener default selects PC for PC-oriented names, two T-spin clears for DT/Albatross, and one T-spin clear otherwise. Search uses the actual board, Hold, queue, board dimensions and rotation/spin rules. Seeded future lookahead is explicitly labeled and can be disabled to restrict analysis to visible Next. Results include multiple routes, complete step lists, previews, optional animation and a live placement outline. Different placements are accepted and cause replanning. These hints do not add another rollback requirement.

Search runs in a cancelable browser Worker with a bounded horizon and time budget. It validates placements with the existing engine, but does not predict gravity/lock timing, calculate unknown-bag probabilities, or prove exhaustive coverage. Use Study or Just think for time to follow the plans. Search does not support generated garbage, bombs or modes without hard drop. The public directory does not contain every opener or all authored later-bag continuations. Details and public research sources are in [OPENERS-IMPLEMENTATION.md](TEMP/OPENERS-IMPLEMENTATION.md).

The header's Convert replay to TETR.IO button converts compatible trainer JSON files independently of the current game. Native exports include native stopwatch, line, piece, key and finesse counter slots. Export and conversion show progress and compatibility errors beside their buttons. The supplied 101-piece / 40-line recording now ends at 0:28.583 in both formats. Line-clear glare is confined to cleared rows.
