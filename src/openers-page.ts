...
export class OpenersPage {
  ...
  private catalog() {
    ...
    el('opener-catalog').replaceChildren(...items.slice(this.catalogPage * 24, (this.catalogPage + 1) * 24).map(opener => {
      ...
      button.append(canvas, title, tag); button.addEventListener('click', () => { this.selected = opener; this.refresh(); el('opener-name').scrollIntoView({ behavior: 'smooth', block: 'start' }); });
      try {
