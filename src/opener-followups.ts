...
export function searchOpenerContinuations(request: OpenerContinuationRequest): ContinuationResult {
  ...
  const started = performance.now(), budget = Math.max(20, Math.min(15000, request.budgetMs ?? 6500)), deadline = started + budget;
  const { context } = request, { rules, settings } = context;
  const depth = Math.max(1, Math.min(14, request.depth));
  const queue = [context.snapshot.falling.symbol, ...context.snapshot.queue.value.slice(0, request.seeded ? depth : rules.nextCount)];
  ...
  for (const stage of stages) for (const mirror of [false, true]) {
    ...
    const solve = (snapshot: EngineSnapshot, cells: Cell[], forced: Cell[], steps: ContinuationStep[], drawn: number): boolean => {
      ...
      if (!cells.length) {
        ...
        if (stage.spinLines && !steps.some(step => step.piece === 't' && step.spin !== 'none' && step.lines === stage.spinLines)) return false;
      ...
      for (const holdFirst of [false, true]) {
        ...
        for (const target of candidates) {
          ...
          const path = findFinesse(engine, before, target); if (!path) continue;
          const rows = clearedRows(before.board, target), result = applyContinuationPath(engine, path);
          if (engine.toppedOut || key(locked) !== key(target)) continue;
          if (forced.length && piece.symbol === 't' && (result.spin === 'none' || result.lines !== stage.spinLines)) continue;
          ...
          const step: ContinuationStep = { scene: { id: `published-${output.checked}`, snapshot, guideSnapshot: holdFirst ? before : undefined, holdFirst, target, path }, after, piece: result.mino, lines: result.lines, spin: result.spin, pc: !!result.lines && !after.board.some(row => row.some(Boolean)) };
