...
import { validateReviveGuidanceResult } from './revive-guidance';
...
self.onmessage = (event: MessageEvent<{ checkpoint: QpCheckpoint; budget: ReviveBudget; validation?: { source: QpCheckpoint; operation: QpOperation } }>) => {
  try { self.postMessage(event.data.validation ? validateReviveGuidanceResult(event.data.checkpoint, event.data.validation.source, event.data.validation.operation) : { result: searchRevive(event.data.checkpoint, 0, event.data.budget) }); }
